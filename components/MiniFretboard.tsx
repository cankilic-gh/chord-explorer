...
const MiniFretboard: React.FC<MiniFretboardProps> = ({ voicing }) => {
  ...
  return (
    <div className="w-10 h-12 bg-white/5 p-1 rounded-sm border border-white/5">
      <div className="relative w-full h-full">
        {/* Nut/Fret 0 line */}
        <div className={`absolute -top-1 left-0 right-0 h-0.5 ${hasOpenStrings ? 'bg-white' : 'bg-white/30'}`}></div>
        ...
        <div className="absolute top-0 left-0 right-0 bottom-0">
          {voicing.filter(pos => pos.fret <= FRET_COUNT).map((pos, i) => {
            ...
            return (
              <div
                key={i}
                className={`absolute w-[0.3rem] h-[0.3rem] rounded-full ${pos.fret === 0 ? 'border border-cyan' : 'bg-cyan shadow-[0_0_4px_#00D4FF]'} transform -translate-x-1/2 -translate-y-1/2`}
                style={{ top, left }}
              ></div>
            );
          })}
