      ...
      <div>
        ...
        <div className="relative w-40 h-40 mx-auto mb-6">
          {NOTES.map((note, i) => {
            ...
            const x = Math.cos(angle) * 60 + 70;
            const y = Math.sin(angle) * 60 + 70;
            ...
            return (
              <button
                ...
                className={`absolute w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-200 transform -translate-x-1/2 -translate-y-1/2
                  ${isSelected ? 'bg-[#4493f8] text-white scale-110' : 'bg-[#21262d] border border-[#30363d] hover:bg-[#30363d]'} font-mono`}
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-[#0d1117] rounded-full border-2 border-[#30363d]"></div>
        </div>
      </div>
